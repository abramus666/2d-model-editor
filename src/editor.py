...
class Application:
   ...
   def save_model(self):
      ...
      for ent in self.entities:
         if ent and (len(ent) > 2):
            dct = data[ent_map[ent[0]]]
            lst = dct[ent[1]] if ent[1] in dct else []
            val = tuple(ent[2:]) if (len(ent[2:]) > 1) else ent[2]
            lst.append(val)
            dct[ent[1]] = lst
      for dct_name in ent_map.values():
      ...
      for ent in self.entities:
         if ent and len(ent) >= 3:
            ...
            if len(ent) == 3: # Point or incomplete edge/rectangle/circle.
               ...
               draw_circle(ENTITY_COLOR, x, y, radius, CIRCLE_NVERTS)
               draw_line(ENTITY_COLOR, x, y, x_, y_)
            if ent[1]:
               set_texture(self.font_tex)
               ch_w, ch_h = self.font_glyph_size
               draw_text(ENTITY_COLOR, x, y, ch_w, ch_h, ent[1])
               set_texture(None)
   ...
   def render(self):
      ...
      for vix in self.selected:
         draw_point_circle(SELECT_COLOR, *self.vertex_to_screen_coords(vix))
      if self.select_rect:
         x,y = self.mouse_pos
